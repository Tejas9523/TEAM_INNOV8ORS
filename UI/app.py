...
def confirm_login(token):
    try:
        # Load the token data
        data = serializer.loads(token, salt='login-salt', max_age=60)  # Token expires after 1 hour
        email = data['email']
        phone = data['phone']
...
def dashboard():
    if 'email' in session:
        try:
            ...
            cursor.execute("SELECT * FROM payment_history WHERE customer_id=%s", (profile[0],))
            ...
            user_data = pd.DataFrame({
                ...
                'Loan_Amount': loan,
                'On_Time_Payments': payment[1],
                'Late_Payments': payment[3],
                'Missed_Payments': payment[2],
            ...
            weights = np.array([0.1, 0.15, 0.1, 0.1, 0.05, 0.05, 0.1, 0.15, 0.05, 0.05, 0.05, 0.05, 0.05, 0.1])
            weighted_encoded_data = new_data_scaled * weights

            # Calculate the risk percentage as a sum of weighted features
            risk_percentage = np.sum(weighted_encoded_data, axis=1)
            
            min_risk = 0
            max_risk = 100
            normalized_risk_percentage = (100 * (risk_percentage - min_risk) / (max_risk - min_risk)).astype('int')
            ...
            return render_template('result.html',user=sentence, credit_score=creditscore, profile=profile, loan=loan, allloan=allloan, credit=credit, inquiry=inquiry, debt=debt, risk=normalized_risk_percentage)
